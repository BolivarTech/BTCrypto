package com.bolivartech.crypto.aes;
 ...
 *
 * AES 256 bits encrypter/decrypter BolivarTech Homepage: <a
 * href="http://www.bolivartech.com">http://www.bolivartech.com</a>.
 *
 * Implementation of the AES block cipher in Java. 
 * Handles a single block encryption or decryption, with diagnostic logging of intermediate
 * values if required. 
 ...
 *
 * Realiza la encryptacion y desencryptacion de blockes de datos de 128 bits
 *
 * Usage :
 *
 ...
 * aes = new AES(); 
 * aes.Password(cryptKey); 
 * data = aes.Encrypt(data); 
 * data = aes.Decrypt(data);
 *
 * key : String, 16 bytes (or 24 or 32) 
 * data : String, with length = multiple of 16 : you must add spaces at the end of your data string to obtain
 * a *16 length if length &gt; Block_Size split into Block_Size blocks
 *
 ...
 * 
 * <ul>
 * <li>v3.0.0 - Se realiza la estandarizacion del generador de llaves para usar
 * la clase KaosRand y PasswordSparker de BolivarTech Utils.
...
public class CuaimaAES {
    ...
    /**
     * specify whether diagnostic trace output is required.  Available levels
     * are:<br> 0: no trace info is generated<br> 1: trace major calls with
     * params (setKey, encrypt, decrypt)<br> 2: + trace round values whilst
     * en/decrypting<br> 3: + trace all steps within each round<br> 4: + trace
     * subkey generation<br> 5: + trace static table values<br>
     */
    public int traceLevel = 0;
    /**
     * string which accumulates diagnostic output for display.  Contents are
     * reset on each major call (setKey, encrypt, decrypt) and should be used
     * after each of these calls returns for display.
     */
    ...
    private static final int BLOCK_SIZE = 16;    // AES uses 128-bit (16 byte) key
    private int ROUNDS = 14, // AES has 10-14 rounds
    ...
    private byte[][] Kd;
    /**
     * AES encryption S-box. See FIPS-197 section 5.1.1 or Stallings section
     * 5.2. Note that hex values have been converted to decimal for easy table
    ...
    /**
     * AES decryption S-box. See FIPS-197 section 5.1.1 or Stallings section
     * 5.2. Note that hex values have been converted to decimal for easy table
     * specification in Java.
     */
    ...
    /**
     * AES key schedule round constant table. See FIPS-197 section 5.1.1 or
     * Stallings section 5.2. Note that hex values have been converted to
     * decimal for easy table specification in Java, and that indexes start at
     * 1, hence initial 0 entry.
     */
    ...
    /**
     * multiply two elements of GF(2^8). Using pre-computed log and alog
     * tables for speed.
     *
    ...
    public byte[] Encrypt(byte[] plain) {
        ...
        int i, k, row, col;
        //int  j;  Comentada por Julian porque supuestamente nunca se usa

        traceInfo = "";                // reset trace info
        if (traceLevel > 0) {
            traceInfo = "encryptAES(" + Converter.byte2StringHex(plain) + ")\n";
        }
        ...
        for (int r = 1; r < numRounds; r++) {
            ...
            if (traceLevel > 1) {
                traceInfo += " = " + Converter.byte2StringHex(a);
            }
        }
        ...
        if (traceLevel > 1) {
            traceInfo += "\n  R" + numRounds + " (Key = " + Converter.byte2StringHex(Ker) + ")\t";
        }
    ...
    //......................................................................
    /**
     * AES decrypt 128-bit ciphertext block using key previously set.
     *
     * Follows cipher specification given in FIPS-197 section 5.3 See pseudo
     * code in Fig 5, and details in this section.
     *
     * @param cipher the 128-bit ciphertext block value to decrypt.
     * @return the decrypted 128-bit block plaintext value.
     */
    public byte[] Decrypt(byte[] cipher) {
        ...
        traceInfo = "";                // reset trace info
        if (traceLevel > 0) {
            traceInfo = "decryptAES(" + Converter.byte2StringHex(cipher) + ")";
        }
        ...
        if (traceLevel > 1) {
            traceInfo += "\n  R" + numRounds + " (Key = " + Converter.byte2StringHex(Kdr) + ")\t";
        }
    ...
    //......................................................................
    /**
     * Expand a user-supplied key material into a session key. See FIPS-197
     * Section 5.3 Fig 11 for details of the key expansion. Session keys will
     * be saved in Ke and Kd instance variables, along with numRounds being the
    ...
    public void setKey(byte[] key) {
        ...
        traceInfo = "";            // reset trace info
        if (traceLevel > 0) {
            traceInfo = "setKey(" + Converter.byte2StringHex(key) + ")\n";
        }
        ...
        if (traceLevel > 3) {
            ...
            for (r = 0; r < numRounds + 1; r++) {
                traceInfo += "  R" + r + "t = " + Converter.byte2StringHex(Ke[r]) + "\n";
            }
            traceInfo += "  Decrypt Round keys:\n";
            for (r = 0; r < numRounds + 1; r++) {
                traceInfo += "  R" + r + "t = " + Converter.byte2StringHex(Kd[r]) + "\n";
            }
        }
    }
    ...
    private static void self_test(String hkey, String hplain, String hcipher, int lev) {
        ...
        CuaimaAES testAES = new CuaimaAES();    // create new AES instance to test triple
    ...
    /**
     * AES has 10-14 rounds but you can specify all you want
     *
     * @return int
     */
    ...
    /**
     * AES has 10-14 rounds but you can specify all you want with 10 as min
     *
     * @param rOUNDS
     */
    ...
    /**
     * AES uses 128/192/256-bit (16/24/32 byte) key
     *
     * @return int
     */
    ...
    /**
     *
     * Esta funcion inicializa la llave de 256 bits para el cuaimaAES basado en
     * un password, que es un String. Este password debe tener una longitud
     * minima de 1 caracteres y debe contener al menos una letra en mayuscula,
     * un numero y un caracter de puntuacion. La funcion retorna: 0 se
     * establecieron las semillas -1 longitud del password es menor a 9
     * caracteres -2 el passord no contiene los tipos de caracteres solicitados.
     *
     * @param passw
     * @return
     */
